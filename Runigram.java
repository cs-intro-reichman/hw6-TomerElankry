...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		Color[][] newC = new Color[image.length][image[0].length];
		for(int i=0;i<image.length;i++)
		{
			for(int j=0;j<image[0].length;j++)
			{
				newC[i][j] = image[i][image[0].length-j-1];
			}
		}
		return newC;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i=0;i<image.length;i++)
		{
			for(int j=0;j<image[0].length;j++)
			{
				newC[i][j] = image[image.length-i-1][j];
			}
		}
		return newC;
	}
	...
	private static Color luminance(Color pixel) {
		Color lum = new Color((int)(pixel.getRed()*0.299),(int)(pixel.getGreen()*0.587),(int)(pixel.getBlue()*0.114));
		return lum;
	}
